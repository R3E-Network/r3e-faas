...
impl RandomProvider {
    ...
    async fn generate_blockchain_random(
        &self,
        min: u64,
        max: u64,
        count: u32,
        ...
        if let Some(client) = &self.neo_client {
            ...
            Err(OracleError::Provider(
                "Neo client not available for blockchain randomness".to_string(),
            ))
        }
    }

    /// Generate VRF-based random numbers
    async fn generate_vrf_random(
        &self,
        min: u64,
        max: u64,
        count: u32,
