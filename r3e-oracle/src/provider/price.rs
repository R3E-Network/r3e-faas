...
impl PriceProvider {
    ...
    async fn get_price_from_coingecko(&self, symbol: &str) -> Result<PriceData, OracleError> {
        ...
        let response =
            self.client.get(&url).send().await.map_err(|e| {
                OracleError::Provider(format!("CoinGecko API request failed: {}", e))
            })?;
        ...
        let data: serde_json::Value = response.json().await.map_err(|e| {
            OracleError::Provider(format!("Failed to parse CoinGecko response: {}", e))
        })?;
    ...
    async fn get_price_from_binance(&self, symbol: &str) -> Result<PriceData, OracleError> {
        ...
        let price_data: BinancePrice = response.json().await.map_err(|e| {
            OracleError::Provider(format!("Failed to parse Binance response: {}", e))
        })?;
