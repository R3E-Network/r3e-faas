...
impl AuthService {
    ...
    pub async fn register_api_key(&self, api_key: ApiKey) -> Result<(), OracleError> {
        ...
        let rate_limiter = Arc::new(RateLimiter::direct(Quota::per_minute(
            std::num::NonZeroU32::new(api_key.rate_limit).unwrap_or(std::num::NonZeroU32::new(60).unwrap()),
        )));
        
        // Store the API key and rate limiter
        self.api_keys.write().await.insert(api_key.id.clone(), api_key);
        self.rate_limiters.write().await.insert(api_key.id.clone(), rate_limiter);
    ...
    pub async fn authenticate(&self, api_key_id: &str, signature: &str, timestamp: u64, payload: &str) -> Result<ApiKey, OracleError> {
        // Get the API key
        let api_key = self.api_keys.read().await.get(api_key_id).cloned()
        ...
        if let Some(expires_at) = api_key.expires_at {
            ...
            if now > expires_at {
                return Err(OracleError::Authentication("API key has expired".to_string()));
            }
        }
        ...
        if now - timestamp > 300 {
            return Err(OracleError::Authentication("Request has expired".to_string()));
        }
    ...
    pub async fn generate_token(&self, user_id: &str, api_key_id: &str) -> Result<String, OracleError> {
        // Get the API key
        let api_key = self.api_keys.read().await.get(api_key_id).cloned()
            .ok_or_else(|| OracleError::Authentication("Invalid API key".to_string()))?;
