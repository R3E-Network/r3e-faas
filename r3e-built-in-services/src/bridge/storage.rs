...
use crate::bridge::types::{
    AssetWrapper, BlockchainNetwork, BridgeError, BridgeTransaction, 
    BridgeTransactionStatus, MessageBridge, TokenBridge
};
...
impl MemoryBridgeStorage {
    ...
    pub fn with_defaults() -> Self {
        ...
        for bridge in token_bridges {
            let _ = storage.token_bridges.write().unwrap().insert(bridge.id.clone(), bridge);
        }
        ...
        for bridge in message_bridges {
            let _ = storage.message_bridges.write().unwrap().insert(bridge.id.clone(), bridge);
        }
        ...
        for wrapper in asset_wrappers {
            let _ = storage.asset_wrappers.write().unwrap().insert(wrapper.id.clone(), wrapper);
        }
...
impl BridgeStorage for MemoryBridgeStorage {
    async fn create_transaction(&self, transaction: BridgeTransaction) -> Result<(), BridgeError> {
        let mut transactions = self.transactions.write().map_err(|e| {
            BridgeError::Storage(format!("Failed to acquire write lock: {}", e))
        })?;
        
        // Check if the transaction already exists
        if transactions.contains_key(&transaction.id) {
            return Err(BridgeError::InvalidInput(format!("Transaction already exists: {}", transaction.id)));
        }
    ...
    async fn get_transaction(&self, transaction_id: &str) -> Result<BridgeTransaction, BridgeError> {
        let transactions = self.transactions.read().map_err(|e| {
            BridgeError::Storage(format!("Failed to acquire read lock: {}", e))
        })?;
    ...
    async fn update_transaction(&self, transaction: BridgeTransaction) -> Result<(), BridgeError> {
        ...
        if !transactions.contains_key(&transaction.id) {
            return Err(BridgeError::NotFound(format!("Transaction not found: {}", transaction.id)));
        }
    ...
    async fn list_transactions(&self, limit: Option<u32>, offset: Option<u32>) -> Result<Vec<BridgeTransaction>, BridgeError> {
        ...
        let paginated_transactions = all_transactions.into_iter()
            .skip(offset)
            .take(limit)
            .collect();
    ...
    async fn list_transactions_by_status(&self, status: BridgeTransactionStatus, limit: Option<u32>, offset: Option<u32>) -> Result<Vec<BridgeTransaction>, BridgeError> {
        ...
        let mut filtered_transactions: Vec<BridgeTransaction> = transactions.values()
        ...
        let paginated_transactions = filtered_transactions.into_iter()
            .skip(offset)
            .take(limit)
            .collect();
    ...
    async fn get_token_bridges(&self) -> Result<Vec<TokenBridge>, BridgeError> {
        let token_bridges = self.token_bridges.read().map_err(|e| {
            BridgeError::Storage(format!("Failed to acquire read lock: {}", e))
        })?;
    ...
    async fn get_message_bridges(&self) -> Result<Vec<MessageBridge>, BridgeError> {
        let message_bridges = self.message_bridges.read().map_err(|e| {
            BridgeError::Storage(format!("Failed to acquire read lock: {}", e))
        })?;
    ...
    async fn get_asset_wrappers(&self) -> Result<Vec<AssetWrapper>, BridgeError> {
        let asset_wrappers = self.asset_wrappers.read().map_err(|e| {
            BridgeError::Storage(format!("Failed to acquire read lock: {}", e))
        })?;
    ...
    async fn add_token_bridge(&self, bridge: TokenBridge) -> Result<(), BridgeError> {
        let mut token_bridges = self.token_bridges.write().map_err(|e| {
            BridgeError::Storage(format!("Failed to acquire write lock: {}", e))
        })?;
    ...
    async fn add_message_bridge(&self, bridge: MessageBridge) -> Result<(), BridgeError> {
        let mut message_bridges = self.message_bridges.write().map_err(|e| {
            BridgeError::Storage(format!("Failed to acquire write lock: {}", e))
        })?;
    ...
    async fn add_asset_wrapper(&self, wrapper: AssetWrapper) -> Result<(), BridgeError> {
        let mut asset_wrappers = self.asset_wrappers.write().map_err(|e| {
            BridgeError::Storage(format!("Failed to acquire write lock: {}", e))
        })?;
    ...
    async fn update_token_bridge(&self, bridge: TokenBridge) -> Result<(), BridgeError> {
        ...
        if !token_bridges.contains_key(&bridge.id) {
            return Err(BridgeError::NotFound(format!("Token bridge not found: {}", bridge.id)));
        }
    ...
    async fn update_message_bridge(&self, bridge: MessageBridge) -> Result<(), BridgeError> {
        ...
        if !message_bridges.contains_key(&bridge.id) {
            return Err(BridgeError::NotFound(format!("Message bridge not found: {}", bridge.id)));
        }
    ...
    async fn update_asset_wrapper(&self, wrapper: AssetWrapper) -> Result<(), BridgeError> {
        ...
        if !asset_wrappers.contains_key(&wrapper.id) {
            return Err(BridgeError::NotFound(format!("Asset wrapper not found: {}", wrapper.id)));
        }
