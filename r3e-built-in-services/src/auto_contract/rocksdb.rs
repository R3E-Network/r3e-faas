...
impl RocksDBAutoContractStorage {
    ...
    pub async fn new<P: AsRef<Path>>(db_path: P) -> Result<Self, AutoContractError> {
        let db = RocksDBStore::new(db_path)
            .map_err(|e| AutoContractError::Storage(format!("Failed to create RocksDB store: {}", e)))?;
...
impl AutoContractStorage for RocksDBAutoContractStorage {
    async fn store_contract(&self, contract: AutoContract) -> Result<(), AutoContractError> {
        ...
        let value = serde_json::to_vec(&contract)
            .map_err(|e| AutoContractError::Storage(format!("Failed to serialize contract: {}", e)))?;
        ...
        self.db.put(&self.contracts_cf, input)
    ...
    async fn get_contract(&self, id: &str) -> Result<AutoContract, AutoContractError> {
        ...
        match self.db.get(&self.contracts_cf, key) {
            Ok(value) => {
                let contract = serde_json::from_slice::<AutoContract>(&value)
                    .map_err(|e| AutoContractError::Storage(format!("Failed to deserialize contract: {}", e)))?;
                Ok(contract)
            },
            Err(r3e_store::GetError::NoSuchKey) => {
                Err(AutoContractError::NotFound(format!("Contract not found: {}", id)))
            },
            Err(e) => {
                Err(AutoContractError::Storage(format!("Failed to get contract: {}", e)))
            }
        }
    }
    ...
    async fn delete_contract(&self, id: &str) -> Result<(), AutoContractError> {
        ...
        match self.db.get(&self.contracts_cf, key) {
            Ok(_) => {
                self.db.delete(&self.contracts_cf, key)
                    .map_err(|e| AutoContractError::Storage(format!("Failed to delete contract: {}", e)))?;
                Ok(())
            },
            Err(r3e_store::GetError::NoSuchKey) => {
                Err(AutoContractError::NotFound(format!("Contract not found: {}", id)))
    ...
    async fn list_user_contracts(&self, user_id: &str) -> Result<Vec<AutoContract>, AutoContractError> {
        ...
        let output = self.db.scan(&self.contracts_cf, input)
    ...
    async fn list_contracts_by_trigger(&self, trigger_type: &str) -> Result<Vec<AutoContract>, AutoContractError> {
        let input = r3e_store::ScanInput {
            start_key: &[],
            start_exclusive: false,
            end_key: &[],
    ...
    async fn store_execution(&self, execution: AutoContractExecution) -> Result<(), AutoContractError> {
        let key = execution.id.as_bytes();
        let value = serde_json::to_vec(&execution)
            .map_err(|e| AutoContractError::Storage(format!("Failed to serialize execution: {}", e)))?;
        ...
        self.db.put(&self.executions_cf, input)
    ...
    async fn get_execution(&self, id: &str) -> Result<AutoContractExecution, AutoContractError> {
        ...
        match self.db.get(&self.executions_cf, key) {
            Ok(value) => {
                let execution = serde_json::from_slice::<AutoContractExecution>(&value)
                    .map_err(|e| AutoContractError::Storage(format!("Failed to deserialize execution: {}", e)))?;
                Ok(execution)
            },
            Err(r3e_store::GetError::NoSuchKey) => {
                Err(AutoContractError::NotFound(format!("Execution not found: {}", id)))
            },
            Err(e) => {
                Err(AutoContractError::Storage(format!("Failed to get execution: {}", e)))
            }
        }
    }
    
    async fn list_contract_executions(&self, contract_id: &str) -> Result<Vec<AutoContractExecution>, AutoContractError> {
        let input = r3e_store::ScanInput {
            start_key: &[],
            start_exclusive: false,
            end_key: &[],
        ...
        let output = self.db.scan(&self.executions_cf, input)
