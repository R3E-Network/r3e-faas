...
use crate::auto_contract::types::{
    AutoContract, AutoContractError, AutoContractExecution, AutoContractExecutionStatus,
    AutoContractTrigger, AutoContractTriggerType,
};
...
pub trait AutoContractService: Send + Sync {
    ...
    async fn delete_contract(&self, id: &str, user_id: &str) -> Result<(), AutoContractError>;
...
impl AutoContractServiceImpl {
    ...
    async fn execute_neo_contract(
        ...
        let neo_client = neo_rust::Client::new(
            &std::env::var("NEO_RPC_URL").unwrap_or("http://localhost:40332".to_string()),
        );

        // Build script for contract invocation
        let script = neo_client
            .build_script(&contract.contract_address, &contract.method, &params)
            .await
            .map_err(|e| AutoContractError::Execution(format!("Failed to build script: {}", e)))?;
        ...
        let tx = neo_client
            ...
            .await
            .map_err(|e| {
                AutoContractError::Execution(format!("Failed to send transaction: {}", e))
            })?;
        ...
        neo_client
            ...
            .await
            .map_err(|e| {
                AutoContractError::Execution(format!("Failed to confirm transaction: {}", e))
            })?;
    ...
    async fn execute_ethereum_contract(
        ...
        log::info!(
            "Executing Ethereum contract: {} method: {}",
            contract.contract_address,
            contract.method
        );
        ...
        let eth_client = web3::Web3::new(
            web3::transports::Http::new(
                &std::env::var("ETH_RPC_URL").unwrap_or("http://localhost:8545".to_string()),
            )
            .map_err(|e| {
                AutoContractError::Execution(format!("Failed to connect to Ethereum: {}", e))
            })?,
        );

        // Create contract instance
        let contract = eth_client
            .eth()
            .contract(
                contract.contract_address.parse().map_err(|e| {
                    AutoContractError::Execution(format!("Invalid contract address: {}", e))
                })?,
                include_bytes!("../abi/auto_contract.json"),
            )
            .map_err(|e| {
                AutoContractError::Execution(format!("Failed to create contract instance: {}", e))
            })?;
        ...
        let tx = contract
            .call(
                ...
                eth_client
                    ...
                    .await
                    .map_err(|e| {
                        AutoContractError::Execution(format!("Failed to get accounts: {}", e))
                    })?
                    .get(0)
                    .cloned(),
        ...
        eth_client
            .eth()
            .transaction_receipt(tx.transaction_hash)
            .await
            .map_err(|e| {
                AutoContractError::Execution(format!("Failed to get transaction receipt: {}", e))
            })?;
    ...
    fn validate_contract(
        ...
        // Validate name
        if name.is_empty() {
            return Err(AutoContractError::InvalidContract(
        ...
        match trigger.trigger_type {
            AutoContractTriggerType::Blockchain => {
                ...
                if !trigger.params.contains_key("network") {
                    return Err(AutoContractError::InvalidTrigger(
            ...
            AutoContractTriggerType::Market => {
                ...
                if !trigger.params.contains_key("asset_pair") {
                    return Err(AutoContractError::InvalidTrigger(
                        "Missing asset_pair parameter".to_string(),
                    ));
                }
                if !trigger.params.contains_key("condition") {
                    return Err(AutoContractError::InvalidTrigger(
                        "Missing condition parameter".to_string(),
                    ));
                }
                if !trigger.params.contains_key("price") {
                    return Err(AutoContractError::InvalidTrigger(
                        "Missing price parameter".to_string(),
                    ));
                }
            }
            AutoContractTriggerType::Custom => {
                ...
                if !trigger.params.contains_key("event_name") {
                    return Err(AutoContractError::InvalidTrigger(
                        "Missing event_name parameter".to_string(),
                    ));
                }
            }
        }
    ...
    async fn check_contract_authorization(
        ...
        if contract.user_id != user_id {
            return Err(AutoContractError::Unauthorized(format!(
                "User {} is not authorized to access contract {}",
                user_id, id
            )));
        }
    ...
    async fn check_execution_authorization(
        ...
        if contract.user_id != user_id {
            return Err(AutoContractError::Unauthorized(format!(
                "User {} is not authorized to access execution {}",
                user_id, id
            )));
        }
...
impl AutoContractService for AutoContractServiceImpl {
    ...
    async fn execute_contract(
        ...
        if !contract.enabled {
            return Err(AutoContractError::Execution(format!(
        ...
        match contract.network.as_str() {
            ...
            "ethereum" => {
                ...
                match self
                    .execute_ethereum_contract(&contract, &trigger_data)
                    .await
                {
                    Ok((tx_hash, result)) => {
                        execution.status = AutoContractExecutionStatus::Success;
    ...
    async fn list_contract_executions(
        ...
        // Check authorization
        self.check_contract_authorization(contract_id, user_id)
            .await?;
