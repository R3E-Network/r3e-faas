...
impl IdentityStorage for MemoryIdentityStorage {
    async fn create_identity(&self, profile: IdentityProfile) -> Result<(), IdentityError> {
        let mut identities = self.identities.write().map_err(|e| {
            IdentityError::Storage(format!("Failed to acquire write lock: {}", e))
        })?;
        
        // Check if the identity already exists
        if identities.contains_key(&profile.did) {
            return Err(IdentityError::AlreadyExists(format!("Identity already exists: {}", profile.did)));
        }
    ...
    async fn get_identity(&self, did: &str) -> Result<IdentityProfile, IdentityError> {
        let identities = self.identities.read().map_err(|e| {
            IdentityError::Storage(format!("Failed to acquire read lock: {}", e))
        })?;
        
        // Get the identity
        identities.get(did).cloned().ok_or_else(|| {
            IdentityError::NotFound(format!("Identity not found: {}", did))
        })
    }
    ...
    async fn update_identity(&self, profile: IdentityProfile) -> Result<(), IdentityError> {
        ...
        if !identities.contains_key(&profile.did) {
            return Err(IdentityError::NotFound(format!("Identity not found: {}", profile.did)));
        }
