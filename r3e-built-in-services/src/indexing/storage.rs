...
use uuid::Uuid;
use crate::indexing::types::{CollectionStats, IndexDefinition, IndexingError, IndexingQuery, IndexingResult};
...
impl MemoryIndexingStorage {
    ...
    fn get_or_create_collection(&self, collection: &str) -> Result<(), IndexingError> {
        let mut collections = self.collections.write().map_err(|e| {
            IndexingError::Storage(format!("Failed to acquire write lock: {}", e))
        })?;
    ...
    fn apply_filter(&self, documents: &HashMap<String, serde_json::Value>, filter: &serde_json::Value) -> Vec<serde_json::Value> {
    ...
    fn apply_sort(&self, mut documents: Vec<serde_json::Value>, sort: &serde_json::Value) -> Vec<serde_json::Value> {
        // This is a simplified implementation that would need to be expanded for a real system
    ...
    fn apply_pagination(&self, documents: Vec<serde_json::Value>, skip: Option<u32>, limit: Option<u32>) -> Vec<serde_json::Value> {
...
impl IndexingStorage for MemoryIndexingStorage {
    async fn query(&self, query: IndexingQuery) -> Result<IndexingResult, IndexingError> {
        let collections = self.collections.read().map_err(|e| {
            IndexingError::Storage(format!("Failed to acquire read lock: {}", e))
        })?;
    ...
    async fn index_data(&self, collection: &str, mut data: serde_json::Value) -> Result<String, IndexingError> {
        // Ensure the collection exists
        self.get_or_create_collection(collection)?;
        
        // Generate an ID if not provided
        let id = if let Some(id_value) = data.get("_id") {
            id_value.as_str().unwrap_or_else(|| Uuid::new_v4().to_string().as_str()).to_string()
        } else {
    ...
    async fn get_collection_stats(&self, collection: &str) -> Result<CollectionStats, IndexingError> {
        ...
        let size_bytes = collection_data.values()
    ...
    async fn create_index(&self, index_def: IndexDefinition) -> Result<(), IndexingError> {
        ...
        let collection_indexes = indexes.entry(index_def.collection.clone()).or_insert_with(Vec::new);
