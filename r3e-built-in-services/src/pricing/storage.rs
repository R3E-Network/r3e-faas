...
use crate::pricing::types::{
    ...
    EcosystemIncentive, NeoEcosystemIntegration, UserBillingProfile,
    BillingRecord, VolumeDiscount
};
...
pub trait PricingStorage: Send + Sync {
    ...
    async fn get_resource_pricing(&self, resource_type: ResourceType, tier: PricingTier) -> Result<ResourcePricing, PricingError>;
    ...
    async fn get_subscription_model(&self, subscription_id: &str) -> Result<SubscriptionModel, PricingError>;
    async fn get_all_subscription_models(&self) -> Result<Vec<SubscriptionModel>, PricingError>;
    async fn get_subscription_models_by_tier(&self, tier: PricingTier) -> Result<Vec<SubscriptionModel>, PricingError>;
    async fn get_subscription_models_by_type(&self, subscription_type: SubscriptionType) -> Result<Vec<SubscriptionModel>, PricingError>;
    async fn add_subscription_model(&self, model: SubscriptionModel) -> Result<(), PricingError>;
    async fn update_subscription_model(&self, model: SubscriptionModel) -> Result<(), PricingError>;
    
    // Value-added service methods
    async fn get_value_added_service(&self, service_id: &str) -> Result<ValueAddedService, PricingError>;
    async fn get_all_value_added_services(&self) -> Result<Vec<ValueAddedService>, PricingError>;
    async fn get_value_added_services_by_tier(&self, tier: PricingTier) -> Result<Vec<ValueAddedService>, PricingError>;
    
    // Ecosystem incentive methods
    async fn get_ecosystem_incentive(&self, incentive_id: &str) -> Result<EcosystemIncentive, PricingError>;
    async fn get_all_ecosystem_incentives(&self) -> Result<Vec<EcosystemIncentive>, PricingError>;
    
    // Neo ecosystem integration methods
    async fn get_neo_ecosystem_integration(&self, integration_id: &str) -> Result<NeoEcosystemIntegration, PricingError>;
    async fn get_all_neo_ecosystem_integrations(&self) -> Result<Vec<NeoEcosystemIntegration>, PricingError>;
    
    // User billing profile methods
    async fn get_user_billing_profile(&self, user_id: &str) -> Result<UserBillingProfile, PricingError>;
    async fn create_user_billing_profile(&self, profile: UserBillingProfile) -> Result<(), PricingError>;
    async fn update_user_billing_profile(&self, profile: UserBillingProfile) -> Result<(), PricingError>;
    
    // Billing record methods
    async fn get_billing_record(&self, record_id: &str) -> Result<BillingRecord, PricingError>;
    async fn create_billing_record(&self, user_id: &str, record: BillingRecord) -> Result<(), PricingError>;
...
impl MemoryPricingStorage {
    ...
    pub fn with_defaults() -> Self {
        ...
        let resource_pricing = vec![
            ...
            ResourcePricing {
                ...
                volume_discounts: vec![
                    VolumeDiscount { threshold: 10000000, discount_percentage: 10.0 }, // 10% off after 10000 seconds
                    VolumeDiscount { threshold: 100000000, discount_percentage: 20.0 }, // 20% off after 100000 seconds
                ],
            },
            ResourcePricing {
                ...
                max_billable_units: None,
                volume_discounts: vec![
                    VolumeDiscount { threshold: 10240, discount_percentage: 10.0 }, // 10% off after 10GB
                    VolumeDiscount { threshold: 102400, discount_percentage: 20.0 }, // 20% off after 100GB
                ],
            },
            ...
            ResourcePricing {
                ...
                max_billable_units: None,
                volume_discounts: vec![
                    VolumeDiscount { threshold: 10000000, discount_percentage: 15.0 }, // 15% off after 10000 seconds
                    VolumeDiscount { threshold: 100000000, discount_percentage: 25.0 }, // 25% off after 100000 seconds
                ],
            },
            ...
            ResourcePricing {
                ...
                max_billable_units: None,
                volume_discounts: vec![
                    VolumeDiscount { threshold: 10000000, discount_percentage: 20.0 }, // 20% off after 10000 seconds
                    VolumeDiscount { threshold: 100000000, discount_percentage: 30.0 }, // 30% off after 100000 seconds
                ],
            },
        ];
        ...
        for pricing in resource_pricing {
            ...
            let _ = storage.resource_pricing.write().unwrap().insert(key, pricing);
        }
        ...
        for model in subscription_models {
            let _ = storage.subscription_models.write().unwrap().insert(model.id.clone(), model);
        }
...
impl PricingStorage for MemoryPricingStorage {
    async fn get_resource_pricing(&self, resource_type: ResourceType, tier: PricingTier) -> Result<ResourcePricing, PricingError> {
        let pricing = self.resource_pricing.read().map_err(|e| {
            PricingError::Storage(format!("Failed to acquire read lock: {}", e))
        })?;
        
        pricing.get(&(resource_type, tier)).cloned().ok_or_else(|| {
            PricingError::NotFound(format!("Resource pricing not found for {:?} in tier {:?}", resource_type, tier))
        })
    }
    ...
    async fn add_resource_pricing(&self, pricing: ResourcePricing) -> Result<(), PricingError> {
        let mut pricing_map = self.resource_pricing.write().map_err(|e| {
            PricingError::Storage(format!("Failed to acquire write lock: {}", e))
        })?;
    ...
    async fn get_subscription_model(&self, subscription_id: &str) -> Result<SubscriptionModel, PricingError> {
        let models = self.subscription_models.read().map_err(|e| {
            PricingError::Storage(format!("Failed to acquire read lock: {}", e))
        })?;
    ...
    async fn get_subscription_models_by_tier(&self, tier: PricingTier) -> Result<Vec<SubscriptionModel>, PricingError> {
        ...
        let filtered_models = models.values()
    ...
    async fn add_subscription_model(&self, model: SubscriptionModel) -> Result<(), PricingError> {
        let mut models = self.subscription_models.write().map_err(|e| {
            PricingError::Storage(format!("Failed to acquire write lock: {}", e))
        })?;
    ...
    async fn update_subscription_model(&self, model: SubscriptionModel) -> Result<(), PricingError> {
        ...
        if !models.contains_key(&model.id) {
            return Err(PricingError::NotFound(format!("Subscription model not found: {}", model.id)));
        }
    ...
    async fn get_value_added_service(&self, service_id: &str) -> Result<ValueAddedService, PricingError> {
        let services = self.value_added_services.read().map_err(|e| {
            PricingError::Storage(format!("Failed to acquire read lock: {}", e))
        })?;
    ...
    async fn get_value_added_services_by_tier(&self, tier: PricingTier) -> Result<Vec<ValueAddedService>, PricingError> {
        ...
        let filtered_services = services.values()
    ...
    async fn get_ecosystem_incentive(&self, incentive_id: &str) -> Result<EcosystemIncentive, PricingError> {
        let incentives = self.ecosystem_incentives.read().map_err(|e| {
            PricingError::Storage(format!("Failed to acquire read lock: {}", e))
        })?;
    ...
    async fn get_neo_ecosystem_integration(&self, integration_id: &str) -> Result<NeoEcosystemIntegration, PricingError> {
        let integrations = self.neo_integrations.read().map_err(|e| {
            PricingError::Storage(format!("Failed to acquire read lock: {}", e))
        })?;
        
        integrations.get(integration_id).cloned().ok_or_else(|| {
            PricingError::NotFound(format!("Neo ecosystem integration not found: {}", integration_id))
        })
    }
    ...
    async fn get_user_billing_profile(&self, user_id: &str) -> Result<UserBillingProfile, PricingError> {
        let profiles = self.user_profiles.read().map_err(|e| {
            PricingError::Storage(format!("Failed to acquire read lock: {}", e))
        })?;
    ...
    async fn create_user_billing_profile(&self, profile: UserBillingProfile) -> Result<(), PricingError> {
        let mut profiles = self.user_profiles.write().map_err(|e| {
            PricingError::Storage(format!("Failed to acquire write lock: {}", e))
        })?;
        
        if profiles.contains_key(&profile.user_id) {
            return Err(PricingError::InvalidInput(format!("User billing profile already exists: {}", profile.user_id)));
        }
    ...
    async fn update_user_billing_profile(&self, profile: UserBillingProfile) -> Result<(), PricingError> {
        ...
        if !profiles.contains_key(&profile.user_id) {
            return Err(PricingError::NotFound(format!("User billing profile not found: {}", profile.user_id)));
        }
    ...
    async fn create_billing_record(&self, user_id: &str, record: BillingRecord) -> Result<(), PricingError> {
        ...
        let mut user_records = self.user_billing_records.write().map_err(|e| {
            PricingError::Storage(format!("Failed to acquire write lock: {}", e))
        })?;
        
        let user_record_ids = user_records.entry(user_id.to_string()).or_insert_with(Vec::new);
    ...
    async fn update_billing_record(&self, record: BillingRecord) -> Result<(), PricingError> {
        ...
        if !records.contains_key(&record.id) {
            return Err(PricingError::NotFound(format!("Billing record not found: {}", record.id)));
        }
