...
pub trait PricingServiceTrait: Send + Sync {
    ...
    async fn subscribe_user(&self, user_id: &str, subscription_id: &str) -> Result<(), PricingError>;
    ...
    async fn add_value_added_service_to_user(&self, user_id: &str, service_id: &str) -> Result<(), PricingError>;
    
    /// Remove value-added service from user
    async fn remove_value_added_service_from_user(&self, user_id: &str, service_id: &str) -> Result<(), PricingError>;
    
    /// Calculate resource usage cost
    async fn calculate_resource_usage_cost(&self, user_id: &str, resource_type: ResourceType, usage: u64) -> Result<f64, PricingError>;
    
    /// Record resource usage
    async fn record_resource_usage(&self, user_id: &str, resource_type: ResourceType, usage: u64) -> Result<(), PricingError>;
    
    /// Generate billing record
    async fn generate_billing_record(&self, user_id: &str) -> Result<BillingRecord, PricingError>;
    
    /// Process payment
    async fn process_payment(&self, billing_record_id: &str, payment_method_id: &str) -> Result<PaymentStatus, PricingError>;
    
    /// Apply ecosystem incentive
    async fn apply_ecosystem_incentive(&self, user_id: &str, incentive_id: &str, data: serde_json::Value) -> Result<(), PricingError>;
}
...
impl<S: PricingStorage> PricingServiceTrait for PricingService<S> {
    ...
    async fn get_subscription_models_by_type(&self, subscription_type: SubscriptionType) -> Result<Vec<SubscriptionModel>, PricingError> {
        self.storage.get_subscription_models_by_type(subscription_type).await
    }
    ...
    async fn get_neo_ecosystem_integration(&self, integration_id: &str) -> Result<NeoEcosystemIntegration, PricingError> {
        self.storage.get_neo_ecosystem_integration(integration_id).await
    }
    ...
    async fn subscribe_user(&self, user_id: &str, subscription_id: &str) -> Result<(), PricingError> {
        ...
        let end_date = match subscription.subscription_type {
            ...
            SubscriptionType::ReservedCapacity => now + subscription.min_commitment_period as u64 * 30 * 24 * 60 * 60, // min_commitment_period months
    ...
    async fn unsubscribe_user(&self, user_id: &str) -> Result<(), PricingError> {
        ...
        if profile.subscription.is_none() {
            return Err(PricingError::InvalidInput(format!("User {} is not subscribed to any plan", user_id)));
        }
        ...
        if let Some(start_date) = profile.subscription_start_date {
            ...
            let min_commitment_seconds = subscription.min_commitment_period as u64 * 30 * 24 * 60 * 60; // min_commitment_period months
    ...
    async fn add_value_added_service_to_user(&self, user_id: &str, service_id: &str) -> Result<(), PricingError> {
        ...
        if profile.value_added_services.contains(&service_id.to_string()) {
    ...
    async fn calculate_resource_usage_cost(&self, user_id: &str, resource_type: ResourceType, usage: u64) -> Result<f64, PricingError> {
        ...
        let pricing = self.storage.get_resource_pricing(resource_type, profile.tier).await?;
    ...
    async fn record_resource_usage(&self, user_id: &str, resource_type: ResourceType, usage: u64) -> Result<(), PricingError> {
        ...
        let current_usage = profile.resource_usage.get(&resource_type).cloned().unwrap_or(0);
        profile.resource_usage.insert(resource_type, current_usage + usage);
    ...
    async fn generate_billing_record(&self, user_id: &str) -> Result<BillingRecord, PricingError> {
        ...
        if let Some(subscription_id) = &profile.subscription {
            ...
            billing_record.items.push(crate::pricing::types::BillingItem {
        ...
        for (resource_type, usage) in &profile.resource_usage {
            // Calculate the cost
            let cost = self.calculate_resource_usage_cost(user_id, *resource_type, *usage).await?;
        ...
        // Store the billing record
        self.storage.create_billing_record(user_id, billing_record.clone()).await?;
