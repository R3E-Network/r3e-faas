...
pub trait TriggerService: Send + Sync {
    ...
    async fn unregister_trigger(&self, trigger_id: &str) -> Result<(), TriggerError>;
...
impl TriggerServiceImpl {
    ...
    fn parse_blockchain_params(
        ...
        let network = params
            ...
            .and_then(|v| v.as_str())
            .ok_or_else(|| {
                TriggerError::InvalidParameters("Missing network parameter".to_string())
            })?
            .to_string();
    ...
    fn parse_time_params(
        ...
        Ok(TimeTriggerParams { cron, timezone })
    }
    ...
    fn parse_market_params(
        ...
        let asset_pair = params
            ...
            .and_then(|v| v.as_str())
            .ok_or_else(|| {
                TriggerError::InvalidParameters("Missing asset_pair parameter".to_string())
            })?
            .to_string();
        ...
        let condition = params
            ...
            .and_then(|v| v.as_str())
            .ok_or_else(|| {
                TriggerError::InvalidParameters("Missing condition parameter".to_string())
            })?
            .to_string();
        ...
        let price = params
            ...
            .and_then(|v| v.as_f64())
            .ok_or_else(|| {
                TriggerError::InvalidParameters("Missing price parameter".to_string())
            })?;
    ...
    fn parse_custom_params(
        ...
        let event_name = params
            ...
            .and_then(|v| v.as_str())
            .ok_or_else(|| {
                TriggerError::InvalidParameters("Missing event_name parameter".to_string())
            })?
            .to_string();
    ...
    async fn evaluate_blockchain_trigger(
        ...
        let event_network = event_data
            ...
            .and_then(|v| v.as_str())
            .ok_or_else(|| {
                TriggerError::EvaluationError("Missing network in event data".to_string())
            })?;
        ...
        if let Some(contract_address) = &blockchain_params.contract_address {
            let event_contract = event_data
                ...
                .and_then(|v| v.as_str())
                .ok_or_else(|| {
                    TriggerError::EvaluationError(
                        "Missing contract_address in event data".to_string(),
                    )
                })?;
        ...
        if let Some(event_name) = &blockchain_params.event_name {
            let event_type = event_data
                ...
                .and_then(|v| v.as_str())
                .ok_or_else(|| {
                    TriggerError::EvaluationError("Missing event_name in event data".to_string())
                })?;
        ...
        if let Some(block_number) = blockchain_params.block_number {
            let event_block = event_data
                ...
                .and_then(|v| v.as_u64())
                .ok_or_else(|| {
                    TriggerError::EvaluationError("Missing block_number in event data".to_string())
                })?;
    ...
    async fn evaluate_time_trigger(
        ...
        let event_timestamp = event_data
            ...
            .and_then(|v| v.as_u64())
            .ok_or_else(|| {
                TriggerError::EvaluationError("Missing timestamp in event data".to_string())
            })?;

        // Convert timestamp to DateTime<Utc>
        let event_time = chrono::DateTime::<chrono::Utc>::from_timestamp(event_timestamp as i64, 0)
    ...
    async fn evaluate_market_trigger(
        ...
        let event_asset_pair = event_data
            ...
            .and_then(|v| v.as_str())
            .ok_or_else(|| {
                TriggerError::EvaluationError("Missing asset_pair in event data".to_string())
            })?;
        ...
        let event_price = event_data
            ...
            .and_then(|v| v.as_f64())
            .ok_or_else(|| {
                TriggerError::EvaluationError("Missing price in event data".to_string())
            })?;
        ...
        match market_params.condition.as_str() {
                ...
                let previous_price = params
                    ...
                    .ok_or_else(|| {
                        TriggerError::InvalidParameters(
                            "Missing previous_price parameter for percent_increase condition"
                                .to_string(),
                        )
                    })?;
            ...
            "percent_decrease" => {
                ...
                let previous_price = params
                    ...
                    .ok_or_else(|| {
                        TriggerError::InvalidParameters(
                            "Missing previous_price parameter for percent_decrease condition"
                                .to_string(),
                        )
                    })?;
            ...
            "range" => {
                ...
                let upper_bound = params
                    ...
                    .ok_or_else(|| {
                        TriggerError::InvalidParameters(
                            "Missing upper_bound parameter for range condition".to_string(),
                        )
                    })?;

                Ok(event_price >= market_params.price && event_price <= upper_bound)
            }
            _ => Err(TriggerError::InvalidParameters(format!(
                "Invalid condition: {}",
                market_params.condition
            ))),
        }
    }
    ...
    async fn evaluate_custom_trigger(
        ...
        if let Some(expected_data) = &custom_params.event_data {
            let actual_data = event_data.get("event_data").ok_or_else(|| {
                TriggerError::EvaluationError("Missing event_data in event data".to_string())
            })?;
            ...
            match matching_mode {
                ...
                "partial" => {
                    ...
                    if let (Some(expected_obj), Some(actual_obj)) =
                        (expected_data.as_object(), actual_data.as_object())
                    {
                ...
                "regex" => {
                    ...
                    if let (Some(pattern), Some(text)) =
                        (expected_data.as_str(), actual_data.as_str())
                    {
                        let regex = regex::Regex::new(pattern).map_err(|e| {
                            TriggerError::InvalidParameters(format!("Invalid regex pattern: {}", e))
                        })?;
                ...
                "jsonpath" => {
                    ...
                    if let Some(path) = expected_data.as_str() {
                        ...
                        let expected_value = params.get("expected_value").ok_or_else(|| {
                            TriggerError::InvalidParameters(
                                "Missing expected_value parameter for jsonpath matching"
                                    .to_string(),
                            )
                        })?;
                        ...
                        return Err(TriggerError::InvalidParameters(
                            "Invalid JSONPath expression".to_string(),
                        ));
                    }
                }
                _ => {
...
impl TriggerService for TriggerServiceImpl {
    ...
    async fn get_trigger(
        ...
        triggers.get(trigger_id).cloned().ok_or_else(|| {
            TriggerError::InvalidParameters(format!("Trigger not found: {}", trigger_id))
        })
    }
        ...
        for (trigger_id, (user_id, function_id, condition)) in triggers.iter() {
            match self.evaluate_trigger(condition, event_data).await {
                Ok(true) => {
                    ...
                    match self
                        .execute_callback(user_id, function_id, trigger_id, event_data)
                        .await
                    {
                        Ok(callback_id) => {
                        ...
                        Err(e) => {
                            log::error!(
                                "Failed to execute callback for trigger {}: {}",
                                trigger_id,
                                e
                            );
                        }
    ...
    async fn execute_callback(
            ...
            .await
        {
            ...
            Err(e) => {
                ...
                Err(TriggerError::CallbackExecution(format!(
