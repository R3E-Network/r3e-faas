...
impl StandardTriggerEvaluator {
    ...
    async fn evaluate_market_trigger(
        ...
        if let Some(target_price) = price {
            match condition {
                ...
                "pct_change" => {
                    ...
                    let previous_price = if let Some((prev_price, _)) = self.get_market_price(event_asset_pair).await {
                        prev_price
                    } else {
                        // Get previous price from event data
                        event_data.get("previous_price").and_then(|v| v.as_f64()).unwrap_or(event_price)
                    };
        ...
        // Update market price cache
        self.update_market_price(event_asset_pair, event_price, event_timestamp).await;
                ...
                "jsonpath" => {
                    ...
                    if let Some(filter_str) = filter.as_str() {
                        ...
                        if filter_str != "*" {
                            ...
                            let path_result = jsonpath_lib::select(actual, filter_str)
