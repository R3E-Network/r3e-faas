...
impl EventProcessor {
    ...
    pub async fn start(&self) -> Result<(), String> {
        ...
        *is_running = true;
        ...
        tokio::spawn(async move {
            info!("Event processor started for user {} and function {}", user_id, function_id);
            
            // Process events until stopped
            while *is_running_clone.read().await {
                // Acquire a task
                match task_source_clone.write().await.acquire_task(user_id, function_id).await {
                    Ok(task) => {
                        // Process the task
            ...
            info!("Event processor stopped for user {} and function {}", user_id, function_id);
        });
    ...
    async fn process_task(
        ...
        let event_data = match task.event {
            ...
            event::Event::NeoContractEvent { ref contract_address, ref events } => {
                json!({
                    "network": "neo",
            ...
            event::Event::EthereumContractEvent { ref contract_address, ref events } => {
                json!({
                    "network": "ethereum",
                    "event_type": "contract_event",
                    "contract_address": contract_address,
        ...
        // Process the event
        match trigger_service.process_event(&event_data, trigger_evaluator.as_ref()).await {
            Ok(callback_ids) => {
            ...
            Err(e) => {
                Err(format!("Failed to process event: {}", e))
            }
        }
    }
}
