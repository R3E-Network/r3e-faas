...
impl EventProcessorService {
    ...
    pub async fn create_processor(
        ...
        let task_source = task_sources.get(task_source_name).cloned().ok_or_else(|| {
            format!("Task source not registered: {}", task_source_name)
        })?;
    ...
    pub async fn start_processor(&self, processor_id: &str) -> Result<(), String> {
        ...
        let processor = processors.get(processor_id).cloned().ok_or_else(|| {
            format!("Processor not found: {}", processor_id)
        })?;
...
mod tests {
    ...
    async fn test_event_processor_service() {
        ...
        let service = EventProcessorService::new(
        ...
        service.register_task_source("mock", task_source.clone()).await.unwrap();
