...
impl MetaTxStorage for InMemoryMetaTxStorage {
    ...
    async fn get_records_by_sender(&self, sender: &str) -> Result<Vec<MetaTxRecord>, Error> {
        let records = self.records.read().await;
        Ok(records.iter().filter(|r| r.request.sender == sender).cloned().collect())
    }
    
    async fn create_record(&self, record: MetaTxRecord) -> Result<(), Error> {
        let mut records = self.records.write().await;
        if records.iter().any(|r| r.request_id == record.request_id) {
            return Err(Error::InvalidParameter(format!("Record already exists with ID: {}", record.request_id)));
        }
        records.push(record);
        Ok(())
    }
    ...
    async fn update_record(&self, record: MetaTxRecord) -> Result<(), Error> {
        ...
        if let Some(index) = records.iter().position(|r| r.request_id == record.request_id) {
            ...
            Err(Error::NotFound(format!("Record not found with ID: {}", record.request_id)))
        }
    }
