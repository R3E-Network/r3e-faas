...
use super::eip712::{
    create_meta_tx_typed_data, verify_eip712_signature, EIP712Domain, MetaTxMessage,
};
use super::storage::MetaTxStorage;
use super::types::{
    BlockchainType, MetaTxRecord, MetaTxRequest, MetaTxResponse, MetaTxStatus, SignatureCurve,
};
...
impl<S: MetaTxStorage> MetaTxService<S> {
    ...
    fn calculate_fee(&self, tx_data: &str, fee_model: &FeeModel) -> u64 {
        match fee_model {
            ...
            FeeModel::Percentage(percentage) => {
                ...
                let tx_value = match neo3::prelude::Transaction::deserialize(
                    &hex::decode(tx_data).unwrap_or_default(),
                ) {
    ...
    async fn verify_signature(&self, request: &MetaTxRequest) -> Result<bool, Error> {
        ...
        match (request.blockchain_type, request.signature_curve) {
            (BlockchainType::NeoN3, SignatureCurve::Secp256r1) => {
                ...
                let public_key = neo3::prelude::PublicKey::from_address(&request.sender)
                ...
                let target_contract = match &request.target_contract {
                    ...
                    None => {
                        return Err(Error::Validation(
                            "Target contract is required for Ethereum transactions".to_string(),
                        ))
                    }
                };
    ...
    async fn get_gas_bank_account(&self, contract_hash: &str) -> Result<String, Error> {
        ...
        let storage = Arc::new(
            crate::gas_bank::rocksdb::RocksDBGasBankStorage::new("./data/gas_bank").await?,
        );
    ...
    async fn relay_transaction(&self, request: &MetaTxRequest) -> Result<String, Error> {
        ...
        match request.blockchain_type {
            BlockchainType::NeoN3 => {
                ...
                let tx = match neo3::prelude::Transaction::deserialize(&tx_data) {
                    Ok(tx) => tx,
                    Err(e) => {
                        return Err(Error::Validation(format!(
                            "Invalid Neo N3 transaction: {}",
                            e
                        )))
                    }
                };
            ...
            BlockchainType::Ethereum => {
                ...
                let gas_estimate = gas_bank_service
                    .estimate_gas(request.tx_data.as_bytes())
                    .await?;
                ...
                let payment = gas_bank_service
                    .pay_for_ethereum_meta_tx(&tx_hash, target_contract, gas_cost)
                    .await?;
        ...
        let record = match self.storage.get_record(request_id).await? {
            ...
            None => {
                return Err(Error::NotFound(format!(
                    "Meta transaction not found with ID: {}",
                    request_id
                )))
            }
        };
