...
impl SecretStorage for RocksDBSecretStorage {
    async fn store_secret(&self, secret: EncryptedSecret) -> Result<(), SecretError> {
        let key = Self::generate_key(&secret.user_id, &secret.function_id, &secret.id).as_bytes().to_vec();
        ...
        self.db.put(&self.secrets_cf, input)
    ...
    async fn get_secret(&self, user_id: &str, function_id: &str, secret_id: &str) -> Result<EncryptedSecret, SecretError> {
        let key = Self::generate_key(user_id, function_id, secret_id).as_bytes().to_vec();
        
        match self.db.get(&self.secrets_cf, &key) {
            Ok(value) => {
                let secret = serde_json::from_slice::<EncryptedSecret>(&value)
                    .map_err(|e| SecretError::Storage(format!("Failed to deserialize secret: {}", e)))?;
                Ok(secret)
            },
            Err(r3e_store::GetError::NoSuchKey) => {
                Err(SecretError::NotFound(format!("Secret not found: {}", secret_id)))
            },
            Err(e) => {
                Err(SecretError::Storage(format!("Failed to get secret: {}", e)))
            }
        }
    }
    ...
    async fn delete_secret(&self, user_id: &str, function_id: &str, secret_id: &str) -> Result<(), SecretError> {
        ...
        match self.db.get(&self.secrets_cf, &key) {
            Ok(_) => {
                self.db.delete(&self.secrets_cf, &key)
    ...
    async fn list_function_secrets(&self, user_id: &str, function_id: &str) -> Result<Vec<EncryptedSecret>, SecretError> {
        ...
        let output = self.db.scan(&self.secrets_cf, input)
