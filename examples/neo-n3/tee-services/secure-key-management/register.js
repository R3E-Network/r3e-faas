/**
 * Registration script for Neo N3 Secure Key Management Example
 * 
 * This script registers the secure key management service function with the Neo N3 FaaS platform.
 */
  ...
  try {
    ...
    const payload = {
      ...
      metadata: {
        ...
        tee: {
          key_management: config.tee.key_management
        }
      }
    };
    
    // Register the function
    console.log('Registering secure key management service function with Neo N3 FaaS platform...');
    ...
    if (response.status === 201) {
      ...
      console.log('TEE Provider:', config.tee.key_management.provider.type);
      
      console.log('\nKey Types:');
      config.tee.key_management.key_types.asymmetric.forEach(keyType => {
        if (keyType.enabled) {
          console.log(`- ${keyType.name}: ${keyType.description}`);
        }
      });
      
      config.tee.key_management.key_types.symmetric.forEach(keyType => {
        if (keyType.enabled) {
          console.log(`- ${keyType.name}: ${keyType.description}`);
        }
      });
      ...
      console.log('\nKey Operations:');
      Object.keys(config.tee.key_management.operations).forEach(operation => {
        const opConfig = config.tee.key_management.operations[operation];
        if (opConfig.enabled) {
          console.log(`- ${operation}: ${opConfig.algorithms.join(', ')}`);
        }
      });
      
      console.log('\nKey Policies:');
      console.log('- Default Policy:');
      console.log(`  - Rotation: ${config.tee.key_management.policies.default.rotation.enabled ? 'Enabled' : 'Disabled'}`);
      console.log(`  - Rotation Period: ${config.tee.key_management.policies.default.rotation.period} days`);
      console.log(`  - Max Operations: ${config.tee.key_management.policies.default.usage.max_operations}`);
      console.log(`  - Max Age: ${config.tee.key_management.policies.default.usage.max_age} days`);
      
      console.log('\nThe secure key management service function is now registered and will start providing secure key management services.');
    } else {
