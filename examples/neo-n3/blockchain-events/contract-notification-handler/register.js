/**
 * Registration script for Neo N3 Contract Notification Event Handler
 * 
 * This script registers the contract notification event handler function with the Neo N3 FaaS platform.
 */
...
async function registerFunction() {
  try {
    ...
    console.log('Registering contract notification handler function with Neo N3 FaaS platform...');
    ...
    if (response.status === 201) {
      ...
      if (config.trigger.config.event_names) {
        console.log('Event Names:');
        config.trigger.config.event_names.forEach(event => {
          console.log(`- ${event}`);
        });
      } else {
        console.log('Event Names: All events');
      }
      
      console.log('\nThe function will be triggered when contract notifications matching these criteria are emitted on the Neo N3 blockchain.');
    } else {
