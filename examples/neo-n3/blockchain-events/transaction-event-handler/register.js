/**
 * Registration script for Neo N3 Transaction Event Handler
 * 
 * This script registers the transaction event handler function with the Neo N3 FaaS platform.
 */
...
async function registerFunction() {
  try {
    ...
    console.log('Registering transaction handler function with Neo N3 FaaS platform...');
    ...
    if (response.status === 201) {
      ...
      // Display information about the transaction filter
      console.log('\nTransaction Filter Configuration:');
      if (config.trigger.config.transaction_types) {
        console.log('Transaction Types:', config.trigger.config.transaction_types.join(', '));
      } else {
        console.log('Transaction Types: All types');
      }
      
      if (config.trigger.config.contract_hashes) {
        console.log('Contract Hashes:', config.trigger.config.contract_hashes.join(', '));
      } else {
        console.log('Contract Hashes: All contracts');
      }
      
      if (config.trigger.config.addresses) {
        console.log('Addresses:', config.trigger.config.addresses.join(', '));
      } else {
        console.log('Addresses: All addresses');
      }
      
      console.log('\nThe function will be triggered when transactions matching these criteria are processed on the Neo N3 blockchain.');
    } else {
