/**
 * Registration script for Neo N3 Custom Oracle Service
 * 
 * This script registers the custom oracle service function with the Neo N3 FaaS platform.
 */
...
async function registerFunction() {
  try {
    ...
    console.log('Registering custom oracle service function with Neo N3 FaaS platform...');
    ...
    if (response.status === 201) {
      ...
      // Display information about the custom oracle service configuration
      console.log('\nCustom Oracle Service Configuration:');
      
      console.log('Data Sources:');
      config.oracle.sources.forEach(source => {
        console.log(`- ${source.name} (Enabled: ${source.enabled}, Type: ${source.type})`);
        if (source.config) {
          console.log(`  Config: ${JSON.stringify(source.config, null, 2).replace(/\n/g, '\n  ')}`);
        }
      });
      
      console.log('\nData Processing:');
      if (config.oracle.processing) {
        if (config.oracle.processing.cache) {
          console.log(`Cache: Enabled=${config.oracle.processing.cache.enabled}, TTL=${config.oracle.processing.cache.ttl}s`);
        }
        
        if (config.oracle.processing.validation) {
          console.log(`Validation: Enabled=${config.oracle.processing.validation.enabled}`);
          if (config.oracle.processing.validation.rules) {
            console.log('  Rules:');
            config.oracle.processing.validation.rules.forEach(rule => {
              console.log(`  - Field: ${rule.field}, Type: ${rule.type}, Min: ${rule.min}, Max: ${rule.max}`);
            });
          }
        }
        
        if (config.oracle.processing.transformation) {
          console.log(`Transformation: Enabled=${config.oracle.processing.transformation.enabled}`);
          if (config.oracle.processing.transformation.conversions) {
            console.log('  Conversions:');
            config.oracle.processing.transformation.conversions.forEach(conversion => {
              console.log(`  - ${conversion.from} to ${conversion.to}: ${conversion.formula}`);
            });
          }
        }
      }
      ...
      console.log('\nThe custom oracle service function is now registered and will start providing data according to the configured schedule.');
    } else {
