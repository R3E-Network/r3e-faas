/**
 * Main function for the Neo N3 Service Management Example
 * 
...
async function handler(request, user, context) {
  try {
    ...
    const serviceName = context?.service?.name || 'example-managed-service';
    ...
    let message = `Welcome to the Neo N3 FaaS Managed Service Example!`;
    
    // Customize the message based on the request
    if (body.name) {
      message = `Hello, ${body.name}! Welcome to the Neo N3 FaaS Managed Service Example!`;
    }
    ...
    const response = {
      ...
      body: {
        ...
        links: {
          auth: `${getBaseUrl(request)}/example-managed-service/auth`,
          data: `${getBaseUrl(request)}/example-managed-service/data`
        }
      }
    };
