/**
 * Example function for Neo N3 FaaS platform management
 * 
 * This function demonstrates a simple HTTP endpoint that can be registered
 * with the Neo N3 FaaS platform and then managed through the platform's APIs.
 */
...
async function handler(request, user, context) {
  try {
    ...
    const functionName = context?.function?.name || 'example-managed-function';
    ...
    let message = `Hello from Neo N3 FaaS platform managed function!`;
    
    // Customize the message based on the request
    if (body.name) {
      message = `Hello, ${body.name}! Welcome to the Neo N3 FaaS platform managed function!`;
    }
