...
use NeoRust::neo_protocol::wallet::Wallet;
use NeoRust::neo_crypto::keys::PrivateKey;
...
impl EndpointService {
    ...
    pub async fn new(config: Config) -> Result<Self, Error> {
        ...
        let gas_bank_storage = Arc::new(RocksDBGasBankStorage::new("./data/gas_bank").await
            .map_err(|e| Error::Database(format!("Failed to create Gas Bank storage: {}", e)))?);
...
impl MetaTxStorage for MockMetaTxStorage {
    ...
    async fn get_record(&self, request_id: &str) -> Result<Option<MetaTxRecord>, r3e_neo_services::Error> {
        let records = self.records.lock().unwrap();
        Ok(records.get(request_id).cloned())
    }
    
    async fn get_records_by_sender(&self, sender: &str) -> Result<Vec<MetaTxRecord>, r3e_neo_services::Error> {
        let records = self.records.lock().unwrap();
