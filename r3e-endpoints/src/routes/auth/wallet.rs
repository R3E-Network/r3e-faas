...
pub async fn connect_wallet(
    ...
    let blockchain_type = match request.blockchain_type.to_lowercase().as_str() {
        ...
        _ => return Err(Error::Validation(format!("Unsupported blockchain type: {}", request.blockchain_type))),
    };
    ...
    service.db_client.store_auth_challenge(
        &challenge_id, 
        &request.address, 
        &blockchain_type, 
        &challenge, 
    ...
    log::info!("Generated challenge for wallet: {} ({})", request.address, blockchain_type);
    Ok(Json(response))
}
...
pub async fn authenticate_wallet(
    ...
    let challenge = service.db_client.get_auth_challenge(&request.challenge_id).await
    ...
    if request.address != challenge.address || request.blockchain_type != challenge.blockchain_type {
        ...
        return Err(Error::Authentication("Address or blockchain type mismatch".into()));
    }
    
    // Get the default signature curve if not provided
    let signature_curve = request.signature_curve.unwrap_or_else(|| {
        match request.blockchain_type.as_str() {
    ...
    let user = service.db_client.find_user_by_wallet_address(
        &request.blockchain_type,
        &request.address
    ).await.map_err(|e| Error::Internal(format!("Database error: {}", e)))?;
    ...
    let user_id = match user {
        ...
        None => {
            ...
            service.db_client.create_wallet_user(
                &user_id,
                &request.address,
                &request.blockchain_type,
            ).await.map_err(|e| Error::Internal(format!("Failed to create user: {}", e)))?;
    ...
    let _ = service.db_client.delete_auth_challenge(&request.challenge_id).await;
    ...
    match blockchain_type {
        "ethereum" => {
            ...
            if let Err(e) = hex::decode(&address[2..]) {
                return Err(Error::Validation(format!("Invalid Ethereum address hex: {}", e)));
            }
        },
        "neo_n3" => {
        ...
        _ => {
            return Err(Error::Validation(format!("Unsupported blockchain type: {}", blockchain_type)));
        }
    }
