...
use crate::provider::{create_default_neo_tee_provider, NeoTeeProvider, TeeProviderImpl};
...
impl TeeServiceImpl {
    ...
    pub fn new() -> Self {
        ...
        #[cfg(feature = "sgx")]
        {
            ...
            providers.insert(TeePlatform::Sgx, Arc::new(provider) as Arc<dyn TeeProvider>);
        }

        #[cfg(feature = "sev")]
        {
            let provider = TeeProviderImpl::default_for_platform(TeePlatform::Sev);
            providers.insert(TeePlatform::Sev, Arc::new(provider) as Arc<dyn TeeProvider>);
        }
    ...
    pub fn get_provider(&self, platform: TeePlatform) -> Result<Arc<dyn TeeProvider>, TeeError> {
        self.providers.get(&platform).cloned().ok_or_else(|| {
            TeeError::Provider(format!(
                "No provider available for platform: {:?}",
                platform
            ))
        })
    }
...
impl TeeService for TeeServiceImpl {
    ...
    ) -> Result<TeeExecutionResponse, TeeError> {
