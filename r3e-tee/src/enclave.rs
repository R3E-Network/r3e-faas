...
impl SimulatedEnclave {
    ...
    async fn execute_js(
        ...
        let input_json = serde_json::to_string(input)
            .map_err(|e| TeeError::Enclave(format!("Failed to serialize input: {}", e)))?;
        ...
        let result = match result {
            Ok(global) => {
                ...
                let result_json: serde_json::Value =
                    serde_v8::from_v8(runtime.v8_isolate(), result).map_err(|e| {
                        TeeError::Enclave(format!("Failed to convert result to JSON: {}", e))
                    })?;
...
impl EnclaveManager {
    ...
    pub async fn create_enclave(
        &self,
        config: EnclaveConfig,
    ) -> Result<Arc<dyn Enclave>, TeeError> {
        // Get the factory for the platform
        let factory = self.factories.get(&config.platform).ok_or_else(|| {
            TeeError::Enclave(format!(
                "No enclave factory available for platform: {:?}",
                config.platform
            ))
        })?;
