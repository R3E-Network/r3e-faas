...
impl TeeProvider for NitroProvider {
    ...
    async fn initialize(&self) -> Result<(), TeeError> {
        ...
        if !nitro_cli_check.status.success() {
            return Err(TeeError::Initialization("nitro-cli is not available".to_string()));
        }
        ...
        self.key_management_service.initialize().await.map_err(|e| {
            TeeError::Initialization(format!("Failed to initialize key management service: {}", e))
        })?;
    ...
    async fn execute(&self, code: &str, input: &serde_json::Value) -> Result<serde_json::Value, TeeError> {
        ...
        let temp_dir = tempfile::tempdir()
            .map_err(|e| TeeError::Execution(format!("Failed to create temporary directory: {}", e)))?;
        ...
        let enclave_id = self.enclave_manager.run_enclave(&config, &code_path, &input_path, &output_path).await
            .map_err(|e| TeeError::Execution(format!("Failed to run enclave: {}", e)))?;
            
        // Wait for the enclave to complete
        self.enclave_manager.wait_for_enclave(&enclave_id).await
        ...
        self.enclave_manager.terminate_enclave(&enclave_id).await
    ...
    async fn generate_attestation(&self) -> Result<AttestationReport, TeeError> {
        ...
        let temp_dir = tempfile::tempdir()
            .map_err(|e| TeeError::Attestation(format!("Failed to create temporary directory: {}", e)))?;
        ...
        std::fs::write(&request_path, request_data.to_string())
            .map_err(|e| TeeError::Attestation(format!("Failed to write attestation request: {}", e)))?;
            
        // Create an output file
        let output_path = temp_dir.path().join("attestation.json");
        
        // Generate the attestation
        self.attestation_service.generate_attestation(&request_path, &output_path).await
    ...
    async fn verify_attestation(&self, attestation: &AttestationReport) -> Result<bool, TeeError> {
        ...
        if attestation.platform != TeePlatform::Nitro {
            return Err(TeeError::Attestation("Attestation is not for Nitro platform".to_string()));
        }
        ...
        std::fs::write(&attestation_path, serde_json::to_string(&attestation.platform_data).unwrap_or_default())
            .map_err(|e| TeeError::Attestation(format!("Failed to write attestation: {}", e)))?;
            
        // Verify the attestation
        let result = self.attestation_service.verify_attestation(&attestation_path).await
