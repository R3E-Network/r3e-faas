...
impl TeeProviderImpl {
    ...
    pub fn default_for_platform(platform: TeePlatform) -> Self {
        ...
        let (name, description) = match platform {
            ...
            TeePlatform::Nitro => ("AWS Nitro Provider", "TEE provider for AWS Nitro Enclaves"),
            TeePlatform::GoogleConfidential => ("Google Confidential Computing Provider", "TEE provider for Google Cloud Confidential Computing"),
            TeePlatform::AzureConfidential => ("Azure Confidential Computing Provider", "TEE provider for Azure Confidential Computing"),
            TeePlatform::Simulated => ("Simulated TEE Provider", "Simulated TEE provider for development and testing"),
        };
...
impl TeeProvider for TeeProviderImpl {
    ...
    async fn verify_attestation(&self, attestation: &AttestationReport) -> Result<bool, TeeError> {
        ...
        let result = self.attestation_service.verify_attestation(attestation).await?;
...
impl NeoTeeProvider {
    ...
    pub async fn execute_neo_request(&self, request: &NeoTeeRequest) -> Result<NeoTeeResponse, TeeError> {
        ...
        let rpc_client = self.neo_rpc_client.as_ref().ok_or_else(|| {
            TeeError::Provider("Neo RPC client not configured".to_string())
        })?;
        ...
        let result = self.base_provider.execute(&code, &serde_json::json!({})).await?;
        
        // Convert the result to a Neo TEE response
        let response = NeoTeeResponse {
            tx_hash: result.get("txid").and_then(|v| v.as_str()).map(|s| s.to_string()),
            result: result.get("result").cloned().unwrap_or(serde_json::json!(null)),
            vm_state: result.get("state").and_then(|v| v.as_str()).unwrap_or("NONE").to_string(),
            gas_consumed: result.get("gas_consumed").and_then(|v| v.as_u64()).unwrap_or(0),
            exception: result.get("exception").and_then(|v| v.as_str()).map(|s| s.to_string()),
            stack: result.get("stack").and_then(|v| v.as_array()).cloned().unwrap_or_default(),
        };
