...
impl ZokratesProvider {
        ...
        if let Some(array) = inputs.as_array() {
            for value in array {
                if let Some(num_str) = value.as_str() {
                    ...
                    return Err(ZkError::InvalidInput(format!("Invalid input value: {}", value)));
                }
            }
        } else {
    ...
    fn deserialize_proof(&self, data: &[u8]) -> ZkResult<ZokratesProof<Bn128Field, G16>> {
        let proof: ZokratesProof<Bn128Field, G16> = serde_json::from_slice(data).map_err(|e| {
...
impl ZkProvider for ZokratesProvider {
    ...
    async fn compile_circuit(&self, code: &str) -> ZkResult<ZkCircuit> {
        ...
        let (compiled_program, compile_info) = zokrates_compile(
            source_path.to_str().unwrap(),
            None,
            Some(&compile_config),
        ).map_err(|e| {
            ZkError::Compilation(format!("Failed to compile Zokrates circuit: {}", e))
        })?;
    ...
    async fn generate_keys(
        ...
        let keypair = proving_system.setup(program).map_err(|e| {
            ZkError::Provider(format!("Failed to setup proving system: {}", e))
        })?;
        
        // Serialize the keys
        let proving_key_data = bincode::serialize(&keypair.pk).map_err(|e| {
    ...
    async fn generate_proof(
        ...
        let pk = bincode::deserialize(&proving_key.key_data).map_err(|e| {
        ...
        // Create the witness
        let witness = program.execute(&witness_inputs).map_err(|e| {
            ZkError::Provider(format!("Failed to execute program: {}", e))
        })?;
        
        // Generate the proof
        let proving_system = Bellman::<Bn128Field, G16>::new();
        let proof = proving_system.generate_proof(program, witness, pk).map_err(|e| {
    ...
    async fn verify_proof(
        ...
        // Verify the proof
        let proving_system = Bellman::<Bn128Field, G16>::new();
        let result = proving_system.verify(zokrates_proof, vk, &inputs).map_err(|e| {
