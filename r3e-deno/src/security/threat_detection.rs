...
impl ThreatDetectionService {
        ...
        if cpu_usage >= self.config.max_cpu_usage_threshold {
            ...
            let event = ThreatEvent {
                ...
                details: format!(
                    ...
                    cpu_usage, self.config.max_cpu_usage_threshold
                ),
        ...
        if memory_usage >= self.config.max_memory_usage_threshold {
            ...
            let event = ThreatEvent {
                ...
                details: format!(
                    ...
                    memory_usage, self.config.max_memory_usage_threshold
                ),
        ...
        if execution_time_secs >= self.config.max_execution_time_threshold {
            ...
            let event = ThreatEvent {
                ...
                details: format!(
                    ...
                    execution_time_secs, self.config.max_execution_time_threshold
                ),
            ...
            for pattern in &self.suspicious_patterns {
                if let Some(captures) = pattern.captures(code) {
                    ...
                    let event = ThreatEvent {
                        ...
                        user_id: user_id.to_string(),
                        timestamp: now,
                        details: format!("Suspicious code pattern detected: {}", matched),
            ...
            for pattern in &self.network_scanning_patterns {
                if let Some(captures) = pattern.captures(code) {
                    ...
                    let event = ThreatEvent {
                        ...
                        user_id: user_id.to_string(),
                        timestamp: now,
                        details: format!("Network scanning pattern detected: {}", matched),
                        severity: ThreatSeverity::High,
                    };
            ...
            for pattern in &self.crypto_mining_patterns {
                if let Some(captures) = pattern.captures(code) {
                    ...
                    let event = ThreatEvent {
                        ...
                        user_id: user_id.to_string(),
                        timestamp: now,
                        details: format!("Crypto mining pattern detected: {}", matched),
    ...
    pub fn record_shell_execution_attempt(&self, user_id: &str, function_id: &str, command: &str) {
        ...
        let event = ThreatEvent {
            ...
            user_id: user_id.to_string(),
            timestamp: now,
            details: format!("Shell execution attempt: {}", command),
            severity: ThreatSeverity::Critical,
        };
    ...
    pub fn record_fs_access_violation(&self, user_id: &str, function_id: &str, path: &str) {
        ...
        let event = ThreatEvent {
            ...
            user_id: user_id.to_string(),
            timestamp: now,
            details: format!("File system access violation: {}", path),
            severity: ThreatSeverity::High,
        };
    ...
    pub fn record_network_access_violation(&self, user_id: &str, function_id: &str, url: &str) {
        ...
        let event = ThreatEvent {
            ...
            user_id: user_id.to_string(),
            timestamp: now,
            details: format!("Unauthorized network access: {}", url),
            severity: ThreatSeverity::High,
        };
...
mod tests {
    ...
    fn test_suspicious_code_detection() {
        ...
        assert!(events
            .iter()
            .all(|e| e.event_type == ThreatEventType::SuspiciousCodePattern));
    ...
    fn test_network_scanning_detection() {
        ...
        assert!(events
            .iter()
            .all(|e| e.event_type == ThreatEventType::NetworkScanning));
    ...
    fn test_crypto_mining_detection() {
        ...
        assert!(events
            .iter()
            .all(|e| e.event_type == ThreatEventType::CryptoMining));

        // Check severity
        assert!(events
            .iter()
            .all(|e| e.severity == ThreatSeverity::Critical));
    }
