...
pub fn op_neo_gas_bank_deposit(
    ...
    let gas_bank_service = GasBankService::new()?;
    let deposit = gas_bank_service
        .deposit(&request.tx_hash, &request.address, request.amount)
        .await?;
...
pub fn op_neo_gas_bank_withdraw(
    ...
    let gas_bank_service = GasBankService::new()?;
    let withdrawal = gas_bank_service
        .withdraw(&request.address, request.amount)
        .await?;
...
pub fn op_neo_gas_bank_pay_gas(#[serde] request: GasBankPayGasRequest) -> Result<String, AnyError> {
    ...
    let gas_bank_service = GasBankService::new()?;
    let transaction = gas_bank_service
        .pay_gas(&request.tx_hash, &request.address, request.amount)
        .await?;
...
pub fn op_neo_abstract_account_create(
    ...
    let account = abstract_account_service
        .create_account(
            &request.owner,
            &request.controllers,
...
pub fn op_neo_abstract_account_execute_operation(
    ...
    let abstract_account_service = AbstractAccountService::new()?;
    let response = abstract_account_service
        .execute_operation(
...
pub fn op_neo_abstract_account_get_operation_status(
    ...
    let abstract_account_service = AbstractAccountService::new()?;
    let status = abstract_account_service
        .get_operation_status(&request_id)
        .await?;
    Ok(status.to_string())
}
