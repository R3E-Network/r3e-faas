...
impl AuthService {
    ...
    pub fn verify_password(&self, password: &str, hash: &str) -> Result<bool, ApiError> {
        let parsed_hash = PasswordHash::new(hash).map_err(|e| {
            ApiError::Authentication(format!("Failed to parse password hash: {}", e))
        })?;
            ...
            if username != user.username {
                let existing_user =
                    sqlx::query_as::<_, User>("SELECT * FROM users WHERE username = $1")
                        ...
                        .await
                        .map_err(|e| {
                            ApiError::Database(format!("Failed to check existing user: {}", e))
                        })?;
    ...
    Config: FromRef<S>,
{
    ...
    async fn from_request_parts(parts: &mut Parts, state: &S) -> Result<Self, Self::Rejection> {
        ...
        let auth_header = headers
            .get("Authorization")
            .ok_or_else(|| {
                ApiError::Authentication("Missing authorization header".to_string()).into_response()
            })?
            .to_str()
            .map_err(|_| {
                ApiError::Authentication("Invalid authorization header".to_string()).into_response()
            })?;

        // Check if the header starts with "Bearer "
        if !auth_header.starts_with("Bearer ") {
            return Err(
                ApiError::Authentication("Invalid authorization header".to_string())
                    .into_response(),
            );
        }
    ...
    PgPool: FromRef<S>,
{
    ...
    async fn from_request_parts(parts: &mut Parts, state: &S) -> Result<Self, Self::Rejection> {
        ...
        let user = sqlx::query_as::<_, User>("SELECT * FROM users WHERE api_key = $1")
            ...
            .ok_or_else(|| {
                ApiError::Authentication("Invalid API key".to_string()).into_response()
            })?;
