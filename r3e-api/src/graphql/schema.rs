...
impl QueryRoot {
    ...
    async fn me(&self, ctx: &Context<'_>) -> Result<UserObject, ApiError> {
        let auth = ctx.data::<Auth>().map_err(|e| {
            ApiError::Authentication(format!("Authentication required: {}", e))
        })?;
    ...
    async fn user(&self, ctx: &Context<'_>, id: Uuid) -> Result<UserObject, ApiError> {
        ...
        let api_service = ctx.data::<Arc<ApiService>>().map_err(|e| {
            ApiError::Server(format!("Failed to get API service: {}", e))
        })?;
    ...
    async fn services(
        ...
        let status = if let Some(status) = status {
            Some(status.parse().map_err(|_| {
                ApiError::Validation(format!("Invalid status: {}", status))
            })?)
        } else {
            None
        };
